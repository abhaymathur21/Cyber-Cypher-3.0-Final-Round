...
async def llm_model(input_string, products_data):
    ...
    if response.text == "order":
        ...
        await query(
            destination="agent1qgmvuf8wuv96ypmsptary360n8qghm80yw0tv39qqk4d5nepgudxzskzqqm",
            message=Message(product=product, quantity=quantity),
        )  # goes to input agent
        processed_result = (
            f"You have successfully bought: {product} and the quantity is {quantity}"
        )
      ...
      Given the above product data, give ONLY product name and id of the product that you recommend the user to buy. respond with all the recommendations in the format: id-name. The given user input: {input_string}
      """
        ]
